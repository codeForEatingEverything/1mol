...
async function main() {
  ...
  // The Earn share is stvUSD; label it as such in the log.
  ...
  console.log("\n[3] Earn / stake vUSD into the Layer 2 strategy (mints stvUSD)");
  ...
  ok("stvUSD shares", ethers.formatUnits(await earnVault.balanceOf(user.address), sDec));
  ok(
  ...
  console.log("\n[5] Earn / unstake stvUSD back to vUSD");
  ...
  ok("stvUSD remaining", ethers.formatUnits(await earnVault.balanceOf(user.address), sDec));
  ...
  // Size the redemption against the WETH the gateway actually holds, at the
  // live share price. A fixed share count breaks once yield has accrued, since
  // each share then claims more WETH than the gateway has on hand.
  const vaultWeth = await weth.balanceOf(d.MajorVault);
  const price = (await majorVault.supportedAssets(d.tokens.WETH)).priceUSD;
  const redeemableUsd = (vaultWeth * price) / 10n ** 18n; // 18-dec USD
  const usdPerShare = await vUsd.convertToAssets(ethers.parseUnits("1", vDec));
  const shareScale = 10n ** BigInt(18 - vDec);
  // Leave headroom so rounding cannot exceed the gateway's balance.
  const majorShares = ((redeemableUsd / shareScale) * 10n ** BigInt(vDec)) / usdPerShare / 2n;
  ok("gateway WETH on hand", ethers.formatUnits(vaultWeth, 18));
